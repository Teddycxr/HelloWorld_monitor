...
class Monitor():
            ...
            if value['leverRate'] <= (1/config.get('leverLimit', 2)*100):
                if time.time() - config.get('warningTime', 0) >= 600:
                    ...
                    if config.get('phoneNumber', 15210166821):
                        try:
                            tb.warningCall(f"{self.contractSymbol} {config['cnName']} 杠杆超过{config.get('leverLimit', 2)}", config.get('phoneNumber', 15210166821))  
                        except:
                            pass
            if value['leverRate'] <= (1/5*100):
                if time.time() - config.get('warningTime', 0) >= 600:
                    tb.warning(
                        f"{config['cnName']} 杠杆超过 {config.get('leverLimit', 5)}!!!", contractSymbol=self.contractSymbol, method='margin')
                    if config.get('phoneNumber', 15210166821):
                        try:
                            tb.warningCall(f"{self.contractSymbol} {config['cnName']} 杠杆超过5倍", 18810925011)
                        except:
                            pass
    ...
    def priceInfo(self):
        ...
        try:
            if self.exchange == 'wbfSpotETP':
                ...
                # print(current)
                self.market = self.accountConfig['account1'][self.accountConfig['account1']['excList'][0]].getMarket()['data']
                print(self.market)
        except:
            self._reset()
            self.log2.write(traceback.format_exc())
            ...
            if 'Spot' in exc:
                ...
                for symbol, vol in bal.items():
                    ...
                    asset = round(1 * config.get(f"{exc}BaseCoinVol").get(symbol, 0)
                              + diff * self.market.get(f"{symbol}/usdt", 0)
                              + asset, 2) 
    ...
    def makerMonitor(self):
        for symbol in self.monitorSymbols:
            try:
                ...
                latency = now - tick[0]
                if latency>=60000:
                    tb.warning(f'超过{int(60000/1000)}s没有最新成交!!', contractSymbol=f"{self.exchange} {symbol}", method='makerMonitor')
    ...
    def getOpenOrdersDepth(self, config):
        ...
        for symbol in self.monitorSymbols:
            try:
                res = config[self.exchange].getOpenOrders(symbol)['data']
            except :
                res = []
            # print(res)
            try:
            ...
            try:
                res = config[self.exchange].getDepth(symbol)['data']
        ...
        if getattr(self, 'model', 'default') == 'default':
            ...
            for i, account in enumerate(self.accountConfig):
                try:
                    ...
                    data_ordersdepth = config['openOrdersDepth']
